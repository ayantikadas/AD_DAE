...
class TimestepBlock(nn.Module):
    ...
    """
    @abstractmethod
    def forward(self, x, emb=None, cond=None, cond_age=None, cond_health=None, lateral=None):
        """
    ...
    def forward(self, x, emb=None, cond=None, cond_age=None, cond_health=None, lateral=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb=emb, cond=cond, cond_age=cond_age,cond_health=cond_health, lateral=lateral)
            else:
                x = layer(x)
        return x
...
#             print("conf.emb_channels, conf.out_channels",conf.emb_channels,  conf.out_channels)
            ...
            if conf.two_cond:
                ...
                self.cond_age_emb_layers = nn.Sequential(
                    nn.SiLU(),
                    linear(conf.cond_emb_channels, conf.out_channels),
                )
                self.cond_health_emb_layers = nn.Sequential(
                    nn.SiLU(),
                    linear(conf.cond_emb_channels, conf.out_channels),
                )
            #############################
    ...
    def forward(self, x, emb=None, cond=None, cond_age=None, cond_health=None, lateral=None):
        ...
        """
#         print("forward of ResBlock")
        return torch_checkpoint(self._forward, (x, emb, cond, cond_age, cond_health, lateral),
...
#                 print("emb_out",emb_out.shape)
            ...
            if self.conf.two_cond:
                ...
                ####################### Dense layer for cond_age #######################
                if cond_age is None:
                    cond_age_out = None
                else:
#                     print("cond_age before cond_age_emb_layers",cond_age.shape)
                    cond_age_out = self.cond_age_emb_layers(cond_age).type(h.dtype)
#                     print("cond_age_out after cond_age_emb_layers",cond_age_out.shape)

                if cond_age_out is not None:
                    while len(cond_age_out.shape) < len(h.shape):
                        cond_age_out = cond_age_out[..., None]
                #######################################################################
                
                ####################### Dense layer for cond_health #######################
                if cond_health is None:
                    cond_health_out = None
                else:
#                     print("cond_health before cond_health_emb_layers",cond_health.shape)
                    cond_health_out = self.cond_health_emb_layers(cond_health).type(h.dtype)
#                     print("cond_health_out after cond_health_emb_layers",cond_health_out.shape)

                if cond_health_out is not None:
                    while len(cond_health_out.shape) < len(h.shape):
                        cond_health_out =cond_health_out[..., None]
                #######################################################################
            else:
                cond_out = None
                cond_age_out = None
            ...
            h = apply_conditions(
                ...
                emb=emb_out,
                cond=cond_out,
                cond_age=cond_age_out,
                cond_health = cond_health_out,
...
def apply_conditions(
    ...
    if two_cond:
        ...
        while len(cond.shape) < len(h.shape):
            cond = cond[..., None]
        if cond_age is not None:    
            while len(cond_age.shape) < len(h.shape):
                cond_age = cond_age[..., None]
        # time first
            scale_shifts = [emb, cond, cond_age]
            
        elif (cond_age is not None) and (cond_health is not None):    
            while len(cond_age.shape) < len(h.shape):
                cond_age = cond_age[..., None]
            while len(cond_health.shape) < len(h.shape):
                cond_health = cond_health[..., None]
        # time first
            scale_shifts = [emb, cond, cond_age, cond_health]
        ...
        scale_shifts = [emb]
    #####################################
#     scale_shifts = [emb, cond, emb]
#     print("scale_shifts",len(scale_shifts))
#     print("scale_shifts",scale_shifts[0].shape,scale_shifts[1].shape)
