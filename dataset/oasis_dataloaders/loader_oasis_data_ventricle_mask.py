...
import slice_data_h5_oasis_ventricle_mask as sdl
        ...
        #     list_image_paths.sort()
            for indx in range(0,len(MCI_M_dict['Subject'])):
                ...
                for sub_indx in range(0,len(MCI_M_dict['Subject'])):
                    if (MCI_M_dict['Subject'][sub_indx] == MCI_M_dict['Subject'][indx]) and (MCI_M_dict['Age'][sub_indx] > MCI_M_dict['Age'][indx]) and ((MCI_M_dict['Age'][sub_indx] - MCI_M_dict['Age'][indx])<4.6):
...
#         print('datalist.keys()',datalist.keys())
        
        masklist = self.make_dict_list_from_csv(csv_mask_name,mode)
#         print('masklist.keys()',masklist)
        key_list_image = []
        key_list_label = []
...
#         print(key_list_image,key_list_label)
        ...
        h5cacheds = sdl.H5CachedDataset(datalist,masklist,transforms_,h5cachedir= self.config.dataloader.h5cachedir_,\
                                       ...
                                       ventricle_mask_root_path = '/home/projects/medimg/ayantika/Ayantika/Data_final/oasis_ventricle_mask/')
