...
class slicer():
    ...
    def __init__(self,vol_3d,cmap_='gray',overlay = False):
        self.volume = vol_3d
        self.cmap=cmap_
        if overlay == True:
    ...
    def vol_slice_view(self,vol,slice_view, slice_no_x,slice_no_y,slice_no_z,fig_size_x,fig_size_y):
        ...
        if slice_view == 'x':
            plt.imshow(vol[slice_no_x,:,:],cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()
        elif slice_view == 'y':
            plt.imshow(vol[:,slice_no_y,:],cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()

        elif slice_view == 'z':
            plt.imshow(vol[:,:,slice_no_z],cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()
    ...
    def vol_slice_view_overlay(self,vol1,vol2,weight,slice_view, slice_no_x,slice_no_y,slice_no_z,fig_size_x,fig_size_y):
        plt.figure(figsize=(fig_size_x,fig_size_y))
        if slice_view == 'x':
            plt.imshow((weight*vol2[slice_no_x,:,:]) + ((1-weight)*vol1[slice_no_x,:,:]),cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()
        elif slice_view == 'y':
            plt.imshow((weight*vol2[:,slice_no_y,:]) + ((1-weight)*vol1[:,slice_no_y,:]),cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()

        elif slice_view == 'z':
            plt.imshow((weight*vol2[:,:,slice_no_z]) + ((1-weight)*vol1[:,:,slice_no_z]),cmap=self.cmap)
            plt.colorbar(orientation='horizontal')
            plt.show()
