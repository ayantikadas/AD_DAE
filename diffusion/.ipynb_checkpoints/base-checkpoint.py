...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        x_t = torch.cat((x_t,x_start_baseline),1)
        ##########################
        terms = {'x_t': x_t}
...
#                 print("in training_losses inside diffusion/base.py age_diff",age_diff)
                model_forward = model.forward(x=x_t.detach(),
                                              ...
                                              **model_kwargs)
#                 print("in training_losses inside diffusion/base.py model_forward",age_diff)
...
#                     raise NotImplementedError()
    ...
    def p_mean_variance(self,
        ...
        x_t = torch.cat((x_t,x_start_baseline),1)
        with autocast(self.conf.fp16):
            model_forward = model.forward(x=x_t, ### changed from x to x_t
                                          t=self._scale_timesteps(t),
                                          age_diff = age_diff,
                                          health_state = health_state,
                                          x_start_baseline = x_start_baseline,
